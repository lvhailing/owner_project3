...
public class DragImageView extends View {
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		default:
			...
			case MotionEvent.ACTION_MOVE:
				...
				Log.i("GCM", mD2 + "________________X:" + mHandsX + "___Y:" + mHandsY);
				if (mD1 < mD2) {
					...
					if (mDrawableRect.width() < mContext.getResources().getDisplayMetrics().widthPixels * 2) {
						...
						mDrawableRect.set(mDrawableRect.left - offsetwidth, mDrawableRect.top - offsettop,
								mDrawableRect.right + offsetwidth, mDrawableRect.bottom + offsettop);
					...
					if (mDrawableRect.width() > mContext.getResources().getDisplayMetrics().widthPixels / 3) {
						int offsetwidth = 10;
						int offsettop = (int) (offsetwidth / mRation_WH);
						mDrawableRect.set(mDrawableRect.left + offsetwidth, mDrawableRect.top + offsettop,
								mDrawableRect.right - offsetwidth, mDrawableRect.bottom - offsettop);
	...
	public void setBounds() {
		if (isFirst) {
			...
			isFirst = false;
			Log.i("rect1______", mDrawableRect.left + "," + mDrawableRect.top + "," + mDrawableRect.right + ","
					+ mDrawableRect.bottom);
		}
		mDrawable.setBounds(mDrawableRect);
		Log.i("rect2______", mDrawableRect.left + "," + mDrawableRect.top + "," + mDrawableRect.right + ","
				+ mDrawableRect.bottom);
		Log.i("center_______", mDrawableRect.centerX() + "," + mDrawableRect.centerY());
