...
import com.haidehui.photo_preview.ScaleImageView.OnImageEventListener;
import com.haidehui.photo_preview.ScaleImageView.SubsamplingScaleImageView;
...
public class PicturePreviewPageView extends FrameLayout {
    ...
    private void init(Context context) {
        ...
        addView(originImageView, ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.MATCH_PARENT);
    ...
    private static void adjustPictureScale(SubsamplingScaleImageView view, int width, int height) {
        if (height >= LONG_IMG_MINIMUM_LENGTH && height / width >= LONG_IMG_ASPECT_RATIO) {
            float scale = SystemUtil.getInstance().displaySize.x / (float) width;
            float centerX = SystemUtil.getInstance().displaySize.x / 2;
