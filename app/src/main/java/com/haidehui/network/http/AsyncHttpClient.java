...
package com.haidehui.network.http;
...
//        httpClient = new DefaultHttpClient(cm, httpParams);
        ...
        if (mUseWap) {
            HttpHost host = new HttpHost(mProxy, Integer.valueOf(mPort).intValue());
            httpClient.getParams().setParameter(ConnRouteParams.DEFAULT_PROXY, host); // 设置代理
        }
    ...
    /**
     *   2018.9.17
     *   解决6.0 以下手机运行时提示ssl证书过期问题，导致无法连接上服务器，拿不到数据 （本质是忽略证书）
     * @return
     */
