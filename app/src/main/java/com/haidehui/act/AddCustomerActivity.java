...
//        initData();
    ...
    private void initTopTitle() {
        ...
        title.setTitle(getResources().getString(R.string.title_null)).setLogo(R.drawable.icons, false).setIndicator(R.drawable.back).setCenterText(getResources()
             .getString(R.string.title_add_customer)).showMore(false).setOnActionListener(new TitleBar.OnActionListener() {
        ...
        dialog.setDateDialog(new DatePickDialog.MyCallback() {
            ...
            public void processTime(Dialog ad, String selectedDate) {
                ...
                if (tv_date != null && isTimeValue(selectedDate)) {
                    //选择的是正确的时间
                    String time = selectedDate.replace("年", "-").replace("月", "-").replace("日", "");
