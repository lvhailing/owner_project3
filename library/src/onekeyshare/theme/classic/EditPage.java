...
import onekeyshare.PicViewer;
import onekeyshare.ShareCore;
...
//	private LinearLayout llAt;
	...
	public void resultTitle(int result){
		llTitle.setId(result);
	}
...
//		}
	...
	public void resultThumb(int result){
		rlThumb.setId(result);
	}
	// shared image container
	...
	private void genBackground() {
		...
		if (backgroundView != null) {
			try {
				Bitmap bgBm = BitmapHelper.captureView(backgroundView, backgroundView.getWidth(), backgroundView.getHeight());
				bgBm = BitmapHelper.blur(bgBm, 20, 8);
